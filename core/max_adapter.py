# core/max_adapter.py
"""Адаптер для платформы Max (по аналогии с VKAdapter)"""
...
class MaxAdapter:
    ...
    def _register_handlers(self):
        ...
        async def on_bot_started(event: BotStarted):
            """Пользователь нажал кнопку 'Начать' в диалоге"""
    ...
    async def _handle_start(self, event: MessageCreated):
        ...
        # Получаем лимиты и статистику пользователя
        limits = get_user_limits(user_id)
        
        # Простая клавиатура (если поддерживается в maxapi)
        try:
        ...
        welcome_text = (
            ...
            f"📊 <b>Твой тариф:</b> {limits['tariff_name']}\n"
            f"🎬 <b>Мнений сегодня:</b> 0/{limits['opinion_limit']}\n\n"
    ...
    async def _handle_random(self, event: MessageCreated):
        ...
        # Форматируем карточку (используем общую функцию из core/movie.py)
